...
def create_event(name: str, datetime_start: datetime, timezone_str: str) -> Dict[str, str]:
    timezone_format = pytz.timezone(timezone_str)
    datetime_with_tz = timezone_format.localize(datetime_start)
...
def time_until_event(event: Dict[str, str]) -> timedelta:
    ...
    time_until = event["datetime_start"] - now
    return time_until
    pass
...
def change_event_timezone(event: Dict[str, str], new_timezone_str: str) -> Dict[str, str]:
    new_event = event 
    new_event["timezone"] = new_timezone_str
    return new_event
    pass
...
def find_next_event(events: List[Dict[str, str]]) -> Optional[Dict[str, str]]:
    ...
    future_events = [ev for ev in events if ev["datetime_start"] > now]
    return min(future_events, key=lambda ev: ev["datetime_start"]) if future_events else None
    pass
